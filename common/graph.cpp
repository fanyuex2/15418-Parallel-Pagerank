...
//   } while (buffer.size() == 0 || buffer[0] == '#');
  ...
  std::cerr << "num_nodes: " << graph->num_nodes << "\n";
  std::cerr << "num_edges: " << graph->num_edges << "\n";

}

void read_graph_file(std::ifstream& file, int* scratch, int num_nodes, int num_edges)
{
  std::string buffer;
  int idx = 0;
  int* edgeCountsPerVertex = (int*) calloc(num_nodes, sizeof(int));
  int* edgeStart = (int*) calloc(num_edges, sizeof(int));
  int* edgeEnd = (int*) calloc(num_edges, sizeof(int));
  //int* scratch = (int*) malloc(sizeof(int) * (num_nodes + num_edges));
  while(!file.eof())
  {
    buffer.clear();
    std::getline(file, buffer);
  ...
  /* Fill out the first num_nodes elements of scratch
     i.e. the start idx of each vertex's outgoing edge in scratch */
  scratch[0] = 0;
  for (int i = 1; i < num_nodes; i++){
    scratch[i] = scratch[i-1] + edgeCountsPerVertex[i-1];
  }

  /* Fill out the remaining elements of scratch
     i.e. the out going edges of each vertex in order */
  // int** E = (int**)malloc((num_nodes + num_edges) * sizeof(int));
  // // Allocating space for each vertex's outgoing edges
  // for (int i = 0; i < num_nodes; i++){
  //   E[i] = (int*)malloc(edgeCountsPerVertex[i] * sizeof(size_t));
  // }
  int** E = new int*[num_nodes];
  for (int i = 0; i < num_nodes; i++){
    E[i] = new int[edgeCountsPerVertex[i]];
  }

  // int* countTrack = (int*) malloc(num_nodes * sizeof(int));
  // for (int i = 0; i < num_nodes; i++){
  //   countTrack[i] = 0;
  // }
  size_t* countTrack = (size_t*)calloc(num_nodes, sizeof(size_t));
  // for (int i = 0; i < num_nodes; i++){
  //   std::cerr << i << ": " << countTrack[i] << "\n";
  // }
  
  for (int i  = 0; i < num_edges; i++){
    int src = edgeStart[i];
    int des = edgeEnd[i];
    size_t count = countTrack[src];
    // std::cerr << "src: " << src << "\n";
    // std::cerr << "des: " << des << "\n";
  ...
  int idx2 = num_nodes;
  for (int i = 0; i < num_nodes; i++){
    for (int j = 0; j < countTrack[i]; j++){
        scratch[idx2] = E[i][j];
        idx2 += 1;
    }
  }

  free(edgeCountsPerVertex);
  free(edgeStart);
  free(edgeEnd);
  free(countTrack);
  // for (int i = 0; i < num_nodes; i++){
...
Graph load_graph(const char* filename)
{
  ...
  read_graph_file(graph_file, scratch, graph->num_nodes, graph->num_edges);
